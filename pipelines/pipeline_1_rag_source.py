...
- **ルール2：特殊コンテンツは「専用の見出し」の下に配置する**
  ...
  - **表(Table):** `#### 表1：比較データ` のような見出しを付け、その下にMarkdownテーブルを配置します。
  - **フローチャート(Mermaid):** `#### フローチャート：治療の流れ` のような見出しを付け、その下にMermaidコードブロックを配置します。
  - **図や表の要約:** 図や表そのものの解説文は、`#### 図1の要約` のように、図表とは別の見出しを付けてください。
